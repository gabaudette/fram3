...
public class Fram3MetricsLogger : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        _logWriter.WriteLine(
            $"# Thresholds: flush OK<1ms WARN<3ms CRIT>=3ms | dirty OK<=5 WARN<=15 CRIT>15" +
            $" | insert ratio WARN>50%"
        );
    ...
    private void OnMetrics(FrameMetrics m)
    {
        ...
        {
            case >= 3.0:
                severity = Worst(severity, Severity.Bad);
                hints.Add($"flush {flushMs:F1}ms — check Build() cost or reduce dirty nodes");
                break;
            case >= 1.0:
                severity = Worst(severity, Severity.Warning);
                hints.Add($"flush {flushMs:F1}ms — watch for growth");
                break;
        }

        switch (m.DirtyNodesFlushed)
        {
            case > 15:
                severity = Worst(severity, Severity.Bad);
                hints.Add($"{m.DirtyNodesFlushed} dirty nodes — coalesce SetState calls");
                break;
            case > 5:
                severity = Worst(severity, Severity.Warning);
                hints.Add($"{m.DirtyNodesFlushed} dirty nodes");
                break;
        }
        ...
        _logWriter?.WriteLine(
            $"  rebuilt={m.NodesRebuilt} mounted={m.NodesMounted}" +
            $" unmounted={m.NodesUnmounted}"
        );

        _logWriter?.WriteLine(
            $"  ops: +{m.DiffOpsInsert}(insert) ~{m.DiffOpsUpdate}(update)" +
            $" -{m.DiffOpsRemove}(remove) >{m.DiffOpsMove}(move)"
        );
