...
{
    [TestFixture]
    internal sealed class FInheritedElementTests
    {
        ...
        private sealed class TestInherited : FInheritedElement
        {
            public int Value { get; }

            public TestInherited(int value, FElement child, FKey? key = null) : base(key)
            {
                Value = value;
                Child = child;
            }
        ...
        private sealed class TestConsumer : FStatelessElement
        {
            public int? LastValue { get; private set; }

            public TestConsumer(FElement child, FKey? key = null) : base(key)
            {
                _child = child;
            }
        ...
        public void FindInherited_NoAncestor_ReturnsNull()
        {
            var leaf = new TestLeafElement("leaf");
            var consumer = new TestConsumer(leaf);
            FInheritedElement? captured = null;
