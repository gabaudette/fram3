...
{
    [TestFixture]
    internal sealed class FAnimatedContainerTests
    {
        ...
        public void Constructor_ZeroDuration_ThrowsArgumentOutOfRangeException()
        {
            Assert.Throws<ArgumentOutOfRangeException>(() =>
                new FAnimatedContainer(0f, new TestLeafElement("x")));
        }
        ...
        public void Constructor_NullChild_ThrowsArgumentNullException()
        {
            Assert.Throws<ArgumentNullException>(() =>
                new FAnimatedContainer(1f, null!));
        }
        ...
        public void Constructor_StoresProperties()
        {
            ...
            var element = new FAnimatedContainer(
                duration: 0.3f,
                child: child,
        ...
        {
            var child = new TestLeafElement("child");
            var element = new FAnimatedContainer(duration: 1f, child: child, width: 0f);

            var node = _expander.Mount(element, null);
            node.IsDirty = false;

            _expander.UpdateElement(node, new FAnimatedContainer(duration: 1f, child: child, width: 100f));
        ...
        {
            var child = new TestLeafElement("child");
            var decorationA = new FBoxDecoration(Color: FColor.Red);
            var decorationB = new FBoxDecoration(Color: FColor.Blue);
            ...
            _expander.UpdateElement(node, new FAnimatedContainer(
                duration: 1f,
                child: child,
            ...
            _scheduler.Flush(_expander);
            var beforeTick = FAnimationSystem.RegisteredCount;
