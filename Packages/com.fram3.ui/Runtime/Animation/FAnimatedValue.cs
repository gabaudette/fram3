...
{
    /// <summary>
    /// Non-generic interface implemented by all <see cref="FAnimatedValue{T}"/> instances.
    /// Used by <see cref="FImplicitAnimationSnapshot"/> internally to store and interpolate
    /// heterogeneous animated values without knowing their concrete types.
    /// </summary>
    public interface IFAnimatedValue
    {
        /// <summary>
        /// The key that identifies this value within an <see cref="FImplicitAnimation"/>.
        /// </summary>
        string Key { get; }

        /// <summary>
        /// Returns true when the target value differs from <paramref name="other"/>'s target.
        /// Used by <see cref="FImplicitAnimation"/> to detect whether any property changed.
        /// </summary>
    ...
    /// <see cref="FImplicitAnimation"/>. When the target changes between builds,
    ...
    /// <see cref="FImplicitAnimationSnapshot.Get{T}"/>.
    /// </summary>
    /// <typeparam name="T">The type of value to animate.</typeparam>
    public sealed class FAnimatedValue<T> : IFAnimatedValue
    {
        ...
        /// <see cref="FImplicitAnimation"/>'s values list.
        /// </param>
        ...
        /// </exception>
        public FAnimatedValue(string key, T target, Func<T, T, float, T> lerp)
        {
        ...
        {
            if (other is not FAnimatedValue<T> typed)
            {
                return true;
            }
        ...
        {
            var from = previous is FAnimatedValue<T> typed ? typed.Target : Target;
            return _lerp(from, Target, t)!;
        }

        /// <inheritdoc/>
        public IFAnimatedValue WithTarget(object frozenTarget) =>
            new FAnimatedValue<T>(Key, (T)frozenTarget, _lerp);
    }
}